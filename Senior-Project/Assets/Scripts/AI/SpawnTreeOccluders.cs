    ...
    void Start()
    {
        ...
        for (int i = 0; i < Mathf.Pow(size,2); i += 2)
        {
            if (groundComp.terrainData.alphamapTextures[0].GetPixel(i / size, i % size).Equals(new(0, 0, 0, 1)))
            {
                ...
                Instantiate(occluder, new(i / size * Mathf.RoundToInt(groundComp.terrainData.size.x) / size, 0.1f, i % size * Mathf.RoundToInt(groundComp.terrainData.size.z) / size), Quaternion.identity, terrain.transform);
            }
        }
    }
