...
public class RectangleSelection : MonoBehaviour
{
    ...
    void Update()
    {
        if(detectDrag())
        {
            ...
            if (hypo > 0.4f)
            {
                ...
                Vector3 startPoint = new Vector3(_initPos.x, 1, _initPos.y);
                Vector3 endPoint = new Vector3(_curPos.x, 1, _curPos.y);
                Vector3 pointA = new Vector3(_initPos.x + Mathf.Cos(Mathf.PI / -4) * horz, 1, _initPos.y + Mathf.Sin(Mathf.PI / -4) * horz);
                Vector3 pointB = new Vector3(_initPos.x + -Mathf.Cos(7 * Mathf.PI / -4) * vert, 1, _initPos.y + -Mathf.Sin(7 * Mathf.PI / -4) * vert);
    ...
    private void selectTroops()
    {
        ...
        center.y = 0;
        Vector3 halfExtends = new(Mathf.Abs(_pointList[0].x - _pointList[3].x) / 2, 0f, Mathf.Abs(_pointList[0].z - _pointList[3].z) / 2);
